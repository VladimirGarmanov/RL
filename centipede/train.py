...
COMMAND_TIMESTEPS = 180_000_000      # универсал по командам: МАКСИМУМ для плавности и баланса
SKILL_TIMESTEPS = 60_000_000         # специалист с тёплым стартом от универсала
SKILL_SCRATCH_TIMESTEPS = 100_000_000  # специалист с нуля (универсала нет)
...
PPO_PARAMS = {
    "n_steps": 4096,          # шагов на среду за rollout (8 сред -> 32768 на обновление)
    "batch_size": 2048,       # супер-крупные батчи => очень гладкие обновления
    "n_epochs": 5,
    "learning_rate": 5e-5,    # ниже чем раньше: маленькие, осторожные шаги (очень долго = очень аккуратно)
    "gamma": 0.99,
    "gae_lambda": 0.95,
    "clip_range": 0.1,        # жёще чем раньше (0.15 -> 0.1): политика не может прыгать далеко
    "ent_coef": 0.0,
    "vf_coef": 0.5,
    "max_grad_norm": 0.3,     # ещё строже (0.5 -> 0.3): совсем маленькие обновления
    "target_kl": 0.02,        # очень консервативно: останавливает обновления ещё раньше
    "use_sde": True,          # более гладкая exploration для непрерывных сервокоманд
    "sde_sample_freq": 4,
    "verbose": 1,
    "device": "cpu",          # MLP-политика на CPU быстрее, чем на MPS/GPU
    "policy_kwargs": {
        "net_arch": [768, 512],  # ещё больше: было [512, 256]
        "log_std_init": -2.5,    # ещё консервативнее в начале обучения
    },
}
