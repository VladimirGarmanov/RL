...
COMMAND_TIMESTEPS = 40_000_000       # универсал по командам (~сутки на 8-ядерном CPU)
SKILL_TIMESTEPS = 8_000_000          # специалист с тёплым стартом от универсала
SKILL_SCRATCH_TIMESTEPS = 25_000_000  # специалист с нуля (универсала нет)
N_ENVS = 8                   # параллельных симуляций
MAX_EPISODE_STEPS = 1000     # 50 секунд симуляции на эпизод
CHECKPOINT_FREQ = 500_000    # суммарных шагов между чекпоинтами (реже чем раньше)
SEED = 42
...
#
# Философия: плавность походки обеспечивают штрафы в среде (action_rate,
# dof_vel, НЧ-фильтр действий), а НЕ задушенный оптимизатор. Оптимизатору —
# стандартные значения и затухающий lr: быстро учимся в начале, аккуратно
# полируем в конце. Душить lr/clip стоит только если approx_kl снова поплывёт.


def linear_schedule(start: float, end: float):
    """Линейное затухание от start к end по ходу обучения (SB3 передаёт
    progress_remaining: 1.0 в начале -> 0.0 в конце)."""
...
PPO_PARAMS = {
    ...
    "batch_size": 1024,       # 16 минибатчей на эпоху — стабильный градиент без гигантизма
    "n_epochs": 10,
    "learning_rate": linear_schedule(3e-4, 3e-5),  # быстрый старт, плавный финиш
    "gamma": 0.99,
    "gae_lambda": 0.95,
    "clip_range": 0.2,        # стандарт PPO; экстремальные 0.1 лечили симптом, а не причину
    "ent_coef": 0.001,        # лёгкий бонус за энтропию: std не схлопнется раньше времени
    "vf_coef": 0.5,
    "max_grad_norm": 0.5,
    "target_kl": 0.05,        # мягкий предохранитель, а не ручной тормоз (0.02 обрывал эпохи)
    "use_sde": True,          # более гладкая exploration для непрерывных сервокоманд
    "sde_sample_freq": 4,
    "verbose": 1,
    "device": "cpu",          # MLP-политика на CPU быстрее, чем на MPS/GPU
    "policy_kwargs": {
        "net_arch": [512, 256],  # ~196k весов политики — влезает в бюджет Teensy/ESP32
        "log_std_init": -2.0,
    },
}
